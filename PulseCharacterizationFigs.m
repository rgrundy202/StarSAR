...
figure(1)
time = linspace(0,3*T_sym, length(data));
plot(time, abs(data))
...
plot(freqs, abs(fftd));
title("Frequency Spectrum for PSS Sequence")
xlabel("Frequency (Hz)")
ylabel("")
...
figure(6)
cor = xcorr([pss,sss.'], [pss,sss.']);
cor = cor/max(cor);
time = linspace(-2*T_sym,2*T_sym, length(cor));
semilogy(time, abs(cor));
title("Autocorrelation of Concatenated SSS and PSS")
xlabel("Delay time (s)")
...
title("Autocorrelation of Three Frame Transmission")
xlabel("Delay Time (s)")
ylabel("Normalized Intensity (a.u.)")
